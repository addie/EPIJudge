...
def word_break_dp(s, w):
    ...
    for substring_length in range(1, n + 1):
        for start in range(0, n - substring_length + 1):
    ...
    while start_index < n:
        ...
        if start_index == split_index:
            words.append(s[start_index:end_index + 1])
            break
        else:
            words.append(s[start_index:split_index])
...
def is_word_break_possible(given_string, dictionary):
    ...
    return is_word_break_possible_recursive_helper(given_string, dictionary, 0,
                                                   max_word_length, DP)


def is_word_break_possible_recursive_helper(given_string, dictionary, start,
                                            max_word_length, DP):
    ...
    for i in range(start, start + max_word_length):
        if i < len(given_string):
            ...
            if new_word in dictionary:
                continue
            if is_word_break_possible_recursive_helper(
                    given_string, dictionary, i + 1, max_word_length, DP):
                DP[start] = True
                return True
...
def all_possible_word_break_helper(given_string, dictionary, start,
                                   max_word_length, DP):
...
def all_possible_word_breaks(given_string, dictionary):
    DP = dict()
    max_word_length = len(max(dictionary, key=len))
    return all_possible_word_break_helper(given_string, dictionary, 0,
                                          max_word_length, DP)

def dictionary_contains(word):
    return word in {"mobile","samsung","sam","sung","man","mango","icecream","and","go","i","like","ice","cream"}

def all_possible_word_breaks_recursive(given):
    all_possible_word_breaks_recursive_helper(given, '')

def all_possible_word_breaks_recursive_helper(given, result):
    for i in range(1, len(given)+1):
        ...
        if dictionary_contains(prefix):
            if i == len(given):
                result += prefix
                print(result)
            all_possible_word_breaks_recursive_helper(suffix, result + prefix + ' ')
...
if __name__ == '__main__':
    ...
    dictionary = {"i", "like", "ice", "cream", "and", "man", "go", "icecream","mango"}
    given_string = "ilikeicecreamandmango"
    ...
    # all_possible_word_breaks_recursive("ilikeicecreamandmango")
